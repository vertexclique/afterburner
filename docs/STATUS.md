...
| 4 | `require` Resolver cache TTLs surfaced to JS | polish | S | ✅ | `require.cache` keyed by absolute path; `delete require.cache[abs]` evicts and re-evaluates on next require (matches Node's hot-reload idiom). `require.main` populated with `{id, filename, exports, loaded, paths, children}` so the canonical `require.main === module` check works. Coverage in b6 — 5 tests including reload + entry-script invocation. |
